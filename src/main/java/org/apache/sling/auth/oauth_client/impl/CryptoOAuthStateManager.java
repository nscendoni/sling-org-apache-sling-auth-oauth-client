...
public class CryptoOAuthStateManager implements OAuthStateManager {
    ...
    public @NotNull Optional<OAuthCookieValue> toOAuthState(@Nullable State state) {
        ...
        try {
            ...
            if (parts.length == 2) return Optional.of(new OAuthCookieValue(parts[0], parts[1], null));
            else if (parts.length == 3) return Optional.of(new OAuthCookieValue(parts[0], parts[1], parts[2]));
