...
public class TokenAccessImpl implements OAuthTokenAccess {
    ...
    @Override
    public @NotNull OAuthTokenResponse getAccessToken(
        ...
        if (logger.isDebugEnabled()) {
            logger.debug(
                    "Accessing token for connection {} and user {}", connection.name(), request.getUserPrincipal());
        }
        // valid access token present -> return token
        if (token.getState() == TokenState.VALID) {
            if (logger.isDebugEnabled()) {
                logger.debug(
                        "Returning valid access token for connection {} and user {}",
                        connection.name(),
        ...
        if (token.getState() == TokenState.EXPIRED) {
            ...
            if (refreshToken.getState() == TokenState.VALID) {
                if (logger.isDebugEnabled()) {
                    logger.debug(
                            "Refreshing expired access token for connection {} and user {}",
                            connection.name(),
                            request.getUserPrincipal());
                }
        ...
        if (logger.isDebugEnabled()) {
            logger.debug(
                    "No valid access token found for connection {} and user {}",
                    connection.name(),
                    request.getUserPrincipal());
        }
    ...
    @Override
    public @NotNull OAuthTokenResponse clearAccessToken(
            @NotNull ClientConnection connection,
            @NotNull SlingHttpServletRequest request,
            @NotNull String redirectPath) {
        if (logger.isDebugEnabled()) {
            logger.debug(
                    "Clearing access token for connection {} and user {}",
                    connection.name(),
                    request.getUserPrincipal());
        }
    ...
    public void clearAccessToken(@NotNull ClientConnection connection, @NotNull ResourceResolver resolver) {
        if (logger.isDebugEnabled()) {
            logger.debug(
                    "Clearing access token for connection {} and user {}", connection.name(), resolver.getUserID());
        }
